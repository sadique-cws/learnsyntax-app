        ...
        return (
            <>
                ...
                <div className="flex flex-1 flex-col gap-8 p-4 lg:p-6 bg-muted/5">
                    <div className="mb-2">
                        <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-2 block">Administrative Panel</span>
    ...
    return (
        <>
            ...
            <div className="w-full flex flex-1 flex-col gap-10 p-4 lg:p-6">
                <div>
                    <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-2 block">Student Dashboard</span>
                    <h1 className="text-4xl font-medium tracking-tight text-foreground leading-none mb-3">My Learning Center</h1>
                    <p className="text-xs font-medium text-muted-foreground tracking-tight max-w-2xl">Access your academic programs, track assignment progress, and manage your professional certifications from a single interface.</p>
                </div>

                {/* Academic Achievement / Certificates */}
                {enrollments.some((e: any) => e.certificate) && (
                    <section className="bg-primary/5 border border-primary/10 rounded-sm p-6 lg:p-8">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold tracking-tight text-foreground">Academic Achievements</h2>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Your earned professional certifications</p>
                                </div>
                            </div>
                        </div>
                                ...
                                <Card key={enrollment.id} className="border-border rounded-sm shadow-none bg-background hover:border-primary transition-colors group">
                                    <CardContent className="p-5">
                                        <div className="flex items-center gap-4 mb-4">
                                            ...
                                            <div>
                                                <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">{enrollment.certificate.certificate_number}</div>
                                                <h3 className="font-bold text-sm tracking-tight">{enrollment.course.title}</h3>
                                            </div>
                                        </div>
                                        <Button asChild size="sm" className="w-full rounded-sm h-9 font-black uppercase tracking-widest text-[9px] bg-primary text-white">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Enrolled Programs</h2>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            {enrollments.length > 0 ? enrollments.map((enrollment: any) => (
                                <Card key={enrollment.id} className="border-border rounded-sm shadow-none hover:border-primary transition-all bg-background relative overflow-hidden">
                                    <CardContent className="p-6">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-tight text-muted-foreground/60">{enrollment.batch?.type ?? 'Regular'}</span>
                                        </div>
                                        <h3 className="font-bold text-lg tracking-tight mb-1">{enrollment.course.title}</h3>
                                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-6">{enrollment.batch?.name ?? 'Batch Pending'}</p>
                                        
                                        <div className="flex gap-3">
                                            <Button asChild variant="outline" size="sm" className="flex-1 rounded-sm h-10 font-black uppercase tracking-widest text-[9px] border-border hover:bg-muted transition-colors">
                                                <Link href={`/courses/${enrollment.course.slug}`}>Course Info</Link>
                                            </Button>
                                            <Button asChild size="sm" className="flex-1 rounded-sm h-10 font-black uppercase tracking-widest text-[9px] bg-primary text-white shadow-lg shadow-primary/10">
                                ...
                                <div className="col-span-full py-20 text-center border border-dashed border-border bg-muted/5 rounded-sm">
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-6">No active enrollments found</p>
                                    <Button asChild className="rounded-sm h-11 px-8 font-black uppercase tracking-widest text-[10px] bg-primary">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        <section>
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h2 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Financial Ledger</h2>
                            </div>
                            ...
                            <div className="space-y-4">
                                {enrollments.filter((e: any) => e.payment).length > 0 ? (
                                        ...
                                        <div key={enrollment.payment.id} className="p-4 bg-background border border-border rounded-sm hover:border-primary transition-colors">
                                            <div className="flex items-center gap-4 mb-4">
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    <div className="font-bold text-xs truncate uppercase tracking-tight">{enrollment.course.title}</div>
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t border-border/50">
                                                <div>
                                                    ...
                                                    <div className="text-[9px] text-green-600 font-black uppercase tracking-widest">SUCCESSFUL</div>
                                                </div>
                                                <Button asChild variant="outline" size="sm" className="rounded-sm h-8 px-4 font-black uppercase tracking-widest text-[8px] border-border hover:bg-muted">
                                    ...
                                    <div className="py-12 text-center border border-border bg-muted/5 rounded-sm">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">No billing history</p>
                                    </div>
                                )}
                            </div>
                        </section>
    ...
    return (
        <Card className={cn("border-border rounded-sm bg-background shadow-none", isPrimary && "border-primary/20 bg-primary/5")}>
            <CardHeader className="flex flex-row items-center justify-between pb-3 space-y-0">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">{title}</CardTitle>
